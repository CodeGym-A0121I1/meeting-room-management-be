...
public interface IRegistrationHistoryRepository extends JpaRepository<RegistrationHistory, String> {
    @Query("select s.user.fullName, s.room.name, s.dateStart, s.dateEnd, s.timeStart, s.timeEnd, s.isCancel " +
            ...
            "and (substring(s.dateStart,6, 2)=?3 or substring(s.dateEnd,6, 2)=?3) and (substring(s.dateStart,1, 4)=?4 or substring(s.dateEnd,1, 4)=?4)")
    ...
    @Query("select s.user.fullName, s.room.name, s.room.roomType.name, s.dateStart, s.dateEnd, s.timeStart, s.timeEnd, s.isCancel " +
            "from RegistrationHistory s inner join s.room inner join s.room.roomType inner join s.user " +
            "where s.room.name=?1 " +
            "and (substring(s.dateStart,6, 2)=?2 or substring(s.dateEnd,6, 2)=?2) and (substring(s.dateStart,1, 4)=?3 or substring(s.dateEnd,1, 4)=?3)")
